...
export default class Snake {
  ...
  constructor(
    ...
    { snakeWidth = 1, walls = true } = {}
  ) {
    this.ctx = <CanvasRenderingContext2D>canvas.getContext("2d")
    initialize("ws://10.0.0.43/ws")
  ...
  private draw() {
    ...
    this.ctx.fillRect(
      this.food.x * this.pixelSize,
      this.food.y * this.pixelSize,
      this.pixelSize,
      this.pixelSize
    )
  }
  ...
  private isCollision(x: number, y: number) {
    return this.snake.some(
      (segment, idx) =>
        segment.x === x && segment.y === y && idx < this.snake.length - 1
    )
  }
  ...
  private changeDirection(event: KeyboardEvent) {
    ...
    if (
      (event.code == "a" || event.code == "ArrowLeft") &&
      this.direction != "r"
    ) {
      this.direction = "l"
    } else if (
      (event.code == "d" || event.code == "ArrowRight") &&
      this.direction != "l"
    ) {
      this.direction = "r"
    } else if (
      (event.code == "w" || event.code == "ArrowUp") &&
      this.direction != "d"
    ) {
      this.direction = "u"
    } else if (
      (event.code == "s" || event.code == "ArrowDown") &&
      this.direction != "u"
    ) {
      this.direction = "d"
    }
